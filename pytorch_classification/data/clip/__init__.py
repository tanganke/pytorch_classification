...
def _check_module_name(module_name: str):
    assert isinstance(module_name, str), f"module_name must be a string, got {module_name}"
...
class CLIPTemplateFactory:
    ...
    Online Documentation: https://tanganke.github.io/fusion_bench/guides/clip_vit/classification_templates/
    ...
    def get_classnames_and_templates(dataset_name: str):
        ...
        if dataset_name not in CLIPTemplateFactory._dataset_mapping:
            raise ValueError(
                f"Unknown dataset {dataset_name}, available datasets: {CLIPTemplateFactory._dataset_mapping.keys()}."
                "You can register a new dataset using `CLIPTemplateFactory.register_dataset()` method."
            )
    ...
    def register_dataset(
        ...
        *,
        ...
        """
        assert dataset_info is None or (classnames is not None and templates is not None), "You must provide either `dataset_info` or both `classnames` and `templates`."

        if dataset_name in CLIPTemplateFactory._dataset_mapping:
            warnings.warn(f"Dataset {dataset_name} is already registered, overwriting the existing dataset information.")
...
def _load_hf_dataset(dataset_name):
    logger.info(f"Loading dataset {dataset_name} from HuggingFace datasets.")
    # handle datasets with special processing
...
def load_clip_dataset(dataset: str, processor):
    if False:
        # customize loading for datasets that is not in HuggingFace datasets here,
        # by default, we only support loading from HuggingFace datasets
        pass
    else:
